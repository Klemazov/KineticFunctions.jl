...
# ╔═╡ 9b711b28-ce09-4b00-9b87-1364422c151a
begin
	#Кастомная библиотека, содержащая основные функции для кинетики
	push!(LOAD_PATH, raw"D:\JULIA\JULIA_DEPOT\dev\KineticFunctions.jl")
	#using StaticArrays;
...
# ╔═╡ 90abf774-2ae7-4418-be6d-0a13aedb53ff
md"""
уравнение выглядит следующим образом:

$\frac{d\alpha}{dt} = Const\cdot f(\alpha)$
"""

# ╔═╡ cf31e2db-bad8-4dd0-8ad4-d6537ac7e0a7
αₛ = range(0.01,0.99, 100)
...
# ╔═╡ 5d1d0c0b-6c04-4c93-957a-a4b7225a0733
begin
	fnmodel = FNModel(nₛ)
	pnmodel = PNModel(nₛ)
	ptmodel = PTModel(nₛ, mₛ)
	
end

# ╔═╡ 0dc75310-2cd3-4d85-aeb7-053ddc313041
begin
	plot(αₛ,@. fnmodel(αₛ))
	#plot!(αₛ,@. pnmodel(αₛ))
	plot!(αₛ,@. ptmodel(αₛ))
...
# ╔═╡ 7626caf7-b300-4e3e-ab98-cedf20fda1c8
# ╠═╡ disabled = true
#=╠═╡
begin
	const R = 8.31;
	...
	Eaₛ = 1.0;
	Aₛ = 1.0;
	paramsₛ = [Eaₛ,Aₛ, βₛ,nₛ,mₛ,pₛ]
	T(t,β) = β*t; 
	K(t,Ea,A, β) = A*exp(-Ea/(R*T(t,β)))
end
  ╠═╡ =#

# ╔═╡ 7891a9dc-afdf-4272-9306-98a48c7e20b8
# ╠═╡ disabled = true
#=╠═╡
begin
	function odefunction(u,p,t)
		α = u[1]
		Ea,A, β, n,m,p = p
		K(t,Ea,A, β)*pn(α,n)
	end
...
# ╔═╡ de51ff12-38df-4616-a0d9-0742e1f39682
#=╠═╡
begin
	probₛ = ODEProblem(odefunction, α₀, tspanₛ, paramsₛ)
	solₛ = solve(probₛ)
end
  ╠═╡ =#

# ╔═╡ 34301411-bd12-4804-b2aa-4a981cb375c5
#=╠═╡
plot(solₛ)
  ╠═╡ =#
...
manifest_format = "2.0"
project_hash = "14447401b5609a8d374875f4254a784391b2271f"
