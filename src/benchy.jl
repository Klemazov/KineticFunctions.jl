...
# ╔═╡ a69c2670-113b-11f1-90de-2b45ee3387a1
begin
...
# ╔═╡ 8aafbd51-02c6-4e13-bfd3-0eccbdccda07
using BenchmarkTools

# ╔═╡ 5031b447-1a22-486b-b462-d39dd9067d0b
begin
...
# ╔═╡ 9347c990-0d49-459d-bebb-5a08b029e2a9
begin
	folder_path = raw"C:\Users\Kirill\.julia\dev\KineticFunctions.jl\assets\\"
	filenames = ["fn1.txt", "fn5.txt", "fn10.txt", "fn20.txt", "fn30.txt"]

	for (dfname, filename) in zip((:df1,:df5, :df10, :df20, :df30),filenames)
		@eval $dfname = TGDataFrame(TGParser(folder_path*$filename))
	end

end

# ╔═╡ 12b710d2-e577-4acf-b624-906addf2f387
begin
	xdata_combined = vcat(df1.time, df5.time, df10.time, df20.time, df30.time)
	ydata_combined = vcat(df1.mass, df5.mass, df10.mass, df20.mass, df30.mass)
end

# ╔═╡ f51b8777-ff7e-4b02-8ec5-ada22be14a2d
scatter(xdata_combined, ydata_combined)

# ╔═╡ 11619218-04c5-4d92-8573-1527ab35b8ea
begin
	abstract type Model end

	struct FModel{T}<:Model
		params::MVector{1,T}
		FModel(n::T) where T = new{T}(MVector(n))
	end
	(model::FModel)(α::D) where D = α^model.params[1]

	struct KModel{T}<:Model
		params::MVector{2,T}
		KModel(A::T,Ea::T) where T = new{T}(MVector(A,Ea))
	end
	function (model::KModel)(t)
		A = model.params[1]
		Ea = model.params[2]
		A*exp(-Ea/(8.31*t))
	end
...
# ╔═╡ e5eecf71-98c3-45ce-ace7-0ed1bad1f1ef
begin
	abstract type AbstractKineticEquation end
	struct KineticEquiation{K,F}<:AbstractKineticEquation
		k::K
		f::F
		KineticEquiation(k::K,f::F) where {K,F} = new{K, F}(k,f)
	end
	(a::KineticEquiation)(t::D,α::D) where D = a.k(t)*a.f(α) 
end

# ╔═╡ dbc2437f-a699-47bb-bc01-75cfa7259279


# ╔═╡ 802b70a1-a19b-4706-ba3a-d3e2001ea80a
# begin
# 		u0 = [1,0,0]
# 		tspan = (0.1,1100)
# 		function f!(du,u,p,t)
# 			clamp!(u,0,1)
# 			du[1] = -kinetic_equation1(t,u[1])
# 			du[2] = -du[1]-kinetic_equation2(t,u[2])
# 			u[3] = 1-u[1]-u[2]
# 		end
...
# ╔═╡ f84f77f3-dd1a-47ff-be04-d4657b9c7bcb
# begin
# 	a = sol[1,:]
# 	b = sol[2,:]
# 	c = sol[3,:]
# 	mass_rel = @. 100- (100-df.mass[end])*(follreact*(1-a)+(1-follreact)*c)
# end

# ╔═╡ 630d004f-86d0-42fd-8125-d362890770e9


# ╔═╡ e4662ff2-dfc5-4fc3-b3d1-1a826e19da44
# res2 = optimize(loss, Optim.minimizer(res),pswarm)

# ╔═╡ 2591cb7b-a5e8-4ea9-a027-eadb963074d2
md"""
β = $@bind β Slider([1,5,10,20,30], show_value=true)
"""

# ╔═╡ 9cf10880-7cbc-41e1-9e1f-ccd2223dd6cc
md"""
df = $@bind df Select([df1 => "df1",df5 => "df5", df10 => "df10", df20 => "df20",df30 => "df30"])
"""

# ╔═╡ ad1205d5-0b85-4b15-849f-84a5a93c671a
md"""
lnA1 = $@bind lnA1 NumberField(1 : 0.1 : 10, default = 1)

Ea1 = $@bind Ea1 NumberField(10 : 0.1 : 100, default = 10)

n1 = $@bind n1 NumberField(0.1 : 0.1 : 10, default = 1)

lnA2 = $@bind lnA2 NumberField(1 : 0.1 : 10, default = 1)

Ea2 = $@bind Ea2 NumberField(10 : 0.1 : 100, default = 10)

n2 = $@bind n2 NumberField(0.1 : 0.1 : 10, default = 1)

follreact = $@bind follreact NumberField(0 : 0.1 : 1, default = 0.5)
"""

# ╔═╡ 987300a5-45dd-417f-add3-1ef4b06543f9
begin
	const fmodel1 = FModel(n1)
	const kmodel1 = KModel(exp(lnA1),Ea1*1e3)
	const kinetic_equation1 = KineticEquiation(kmodel1, fmodel1)

	const fmodel2 = FModel(n2)
	const kmodel2 = KModel(exp(lnA2),Ea2*1e3)
	const kinetic_equation2 = KineticEquiation(kmodel2, fmodel2)
end

# ╔═╡ df8f6140-7629-4164-9d3f-c6eb0e13e2d8
function loss(params)
	A1, Ea1, n1, A2, Ea2, n2, follreact = params
	u0 = [1,0,0]
	tspan = (0.1,1000)
	
	kinetic_equation1.f.params[1] = n1
	kinetic_equation1.k.params[1] = A1
	kinetic_equation1.k.params[2] = Ea1

	kinetic_equation2.f.params[1] = n2
	kinetic_equation2.k.params[1] = A2
	kinetic_equation2.k.params[2] = Ea2

	function f!(du,u,p,t)
			clamp!(u,0,1)
			du[1] = -kinetic_equation1(β*t,u[1])
			du[2] = -du[1]-kinetic_equation2(β*t,u[2])
			u[3] = 1-u[1]-u[2]
		end


		prob = ODEProblem(f!, u0, tspan)
		sol = solve(prob, DP5(),  abstol = 1e-10,reltol = 1e-5, maxiters=1e10)
    ...
    return norm(df.mass - mass_rel)^2  # Squared error
end

# ╔═╡ 2188f7e2-198c-425f-9bc6-1cc00c518450
begin 
	function opt_solution(params)
	A1, Ea1, n1, A2, Ea2, n2, follreact = params
	u0 = [1,0,0]
	tspan = (0.1,1100)
	...
	function f!(du,u,p,t)
			clamp!(u,0,1)
			du[1] = -kinetic_equation1(5*t,u[1])
			du[2] = -du[1]-kinetic_equation2(5*t,u[2])
			u[3] = 1-u[1]-u[2]
		end

		prob = ODEProblem(f!, u0, tspan)
		sol = solve(prob, DP5(), abstol = 1e-11, reltol = 1e-10, maxiters=1e10)

		a = sol(df5.time)[1,:]
		b = sol(df5.time)[2,:]
		c = sol(df5.time)[3,:]
		mass_rel = @. 100- (100-df5.mass[end])*(follreact*(1-a)+(1-follreact)*c)
	
		return mass_rel
	end
end

# ╔═╡ 38797cc0-baaa-49a3-9e20-fc9f0df4060c
params = [
	exp(lnA1),
	Ea1*1e3,
	n1,
...
# ╔═╡ 60ebcb4a-9857-4538-bd81-04b6620b3769
begin
	lower = [exp(1),
	1*1e3,
	0.1,
...
# ╔═╡ 8945d3d8-0e80-4c48-b752-d254ab84554b
	pswarm = ParticleSwarm(; lower = lower,
                upper = upper,
                n_particles = 50)

# ╔═╡ 9845a481-9cdf-4fde-9e66-ba3cdab0511e
res = optimize(loss, params, Optim.Options(g_tol = 1e-3, iterations = 3000, show_trace = true))

# ╔═╡ 303132db-42bd-4f40-8545-721b9088c104
Optim.minimizer(res)

# ╔═╡ 8475ad0d-8ba6-4464-8ee6-fd89ac07706f
Optim.minimizer(res)

# ╔═╡ e185384f-1574-4e56-af42-b42e1229d08b
begin
	solution1 = opt_solution(Optim.minimizer(res))
	#solution2 = opt_solution(Optim.minimizer(res2))
end

# ╔═╡ 5369674c-de2e-4693-9492-88563147e584
begin
	plot(df1.time, df1.mass, label = "1")
	plot!(df5.time, df5.mass, label = "5")
	plot!(df10.time, df10.mass, label = "10")
	plot!(df20.time, df20.mass, label = "20")
	scatter!(df5.time, solution1, label = "calculation")
end

# ╔═╡ 9ebf0d32-8741-48b9-81a7-7883d4002da0
begin
	plot(sol.t, mass_rel, label = "calculated")
	plot!(df.time, df.mass, label = "true")
end
...
manifest_format = "2.0"
project_hash = "746fdfb8347076aaeb8b05f4cdfb87c6f4fca487"
